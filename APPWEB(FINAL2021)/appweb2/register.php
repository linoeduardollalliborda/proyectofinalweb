...
if(empty($f_name) || empty($l_name) || empty($email) || empty($password) || empty($repassword) ||
	empty($mobile) || empty($address1) || empty($region)|| empty($dni)){
...
..!</b>
	...
	if(!(strlen($mobile) == 9)){
		echo "
			<div class='alert alert-warning'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>El número de móvil debe ser de 9 dígitos</b>
			</div>
		";
		exit();
	}
	...
	if(!preg_match($number,$dni)){
		echo "
			<div class='alert alert-warning'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>El número $dni no es lo mismo</b>
			</div>
		";
		exit();
	}
	...
	if($count_email > 0){
		...
		`password`, `mobile`, `address1`, `region`,`dni`) 
